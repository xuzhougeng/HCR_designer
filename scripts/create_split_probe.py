from src.dual_probe import DualProbeDesigner, ProbeSet, DualProbeConfig,ProbeOutputHandler
import os
import random
...
ANCHOR_SEQ = "TCTCGTTTTCTGAACTTAGC"

def create_config( min_length, max_length, 
                gc_min, gc_max, tm_min, tm_max, 
                min_gap, inner_gap, 
                min_complementary_length, poly_n, kmer_size, min_kmer_count,
                output_dir, blast_db):
    """创建双探针配置对象
    ...
    Returns:
        DualProbeConfig: 配置对象
    """
    return DualProbeConfig(
...
def generate_dual_probe(sequence: str, 
    ...
    BP_ID:str, 
    bridge_probe: str, 
    config: DualProbeConfig):
    """
    为序列创建DualProbe探针, 用于HCR
    ...
    Returns:
    --------
    list: DualProbe探针列表，每个元素包含：
        L: L + TCTCGTTTTCTGAACTTAGC + CGGTATCAAG[10nt]
        R: CTGTTTAAGA + N + brigde_probe[0:16] + (bridge_probe[17:19]的互补序列)  + R
    """
    ...
    designer = DualProbeDesigner(config)
    ...
    with open(readme_path, 'w') as f:
        f.write(f"Dual Probe Design Results\n")
...
def main( name, sequence, gene_id, 
    ...
    print(f"Running Dual probe design for gene {name} with bridge probe {gene_id}")
    generate_dual_probe(sequence, name, bridge_probe_id, bridge_probe, config)
