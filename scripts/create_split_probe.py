...
def create_config( min_length, max_length, 
                gc_min, gc_max, tm_min, tm_max, 
                  min_gap, r_m_gap, min_complementary_length, poly_n, 
                  output_dir, blast_db):
    """创建三探针配置对象
...
def generate_triplet_probe(sequence: str, 
    ...
    output_handler.save_probe_sets(probe_sets, "dual_probe", "dual_probe", config.blast_db)
    ...
    output_file = os.path.join(config.output_dir, "dual_probes.csv")
...
def main( name, sequence, gene_id, 
         min_length, max_length, min_gc, max_gc, min_tm, max_tm, min_gap,
         min_complementary_length, poly_n,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Split probe design tool')
    # 必需参数
